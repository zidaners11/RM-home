...
const SheetsView: React.FC = () => {
  ...
  // Usamos el parámetro widget=true y headers=false para una vista más limpia
  const embedUrl = `https://docs.google.com/spreadsheets/d/${sheetId}/edit?gid=${gid}&rm=minimal`;

  return (
    <div className="fixed inset-0 z-0 flex items-center justify-center pointer-events-none">
      {/* 
        Contenedor al 80% del viewport.
      */}
      <div className="w-[85vw] h-[85vh] pointer-events-auto glass rounded-[48px] overflow-hidden border border-white/10 flex flex-col shadow-[0_0_120px_rgba(0,0,0,0.9)] animate-in zoom-in-95 duration-1000">
        ...
        <div className="px-10 py-6 border-b border-white/10 flex justify-between items-center bg-black/60 backdrop-blur-3xl shrink-0 z-30">
           <div className="flex items-center gap-6">
              ...
              <div>
                 <h3 className="text-lg font-black uppercase tracking-[0.4em] text-white/90">Matriz Logística</h3>
                 <p className="text-[10px] text-white/20 font-mono tracking-tighter uppercase">Nexus OS // Escala_Optimizada_0.8 // GID_{gid}</p>
              </div>
           </div>
           ...
           <div className="flex items-center gap-4">
              <div className="flex flex-col items-end mr-6">
                 ...
                 <span className="text-[8px] text-white/10 font-mono">DENSIDAD_AJUSTADA_AUTO</span>
              </div>
              <button 
              ...
              >
                <span className="text-[10px] font-black uppercase tracking-widest text-white/40 group-hover:text-white">Editor Externo</span>
        ...
        {/* Viewport de la Hoja con Zoom Out mediante Escalado */}
        <div className="flex-1 bg-[#f8f9fa] relative overflow-hidden">
           {isLoading && (
              <div className="absolute inset-0 z-20 glass-dark flex flex-col items-center justify-center gap-8">
                 ...
                 <div className="text-center space-y-2">
                    <p className="text-[10px] text-blue-400 font-black tracking-[0.6em] uppercase">Recalibrando Lentes de Datos</p>
                    <p className="text-[9px] text-white/20 font-mono italic">Ajustando factor de escala a 0.8x...</p>
                 </div>
              </div>
           )}
           ...
           {/* 
             Técnica de Zoom: 
             1. El iframe mide 125% del contenedor (1 / 0.8 = 1.25)
             2. Se escala a 0.8
             3. transform-origin en top-left (0 0)
           */}
           <div className="w-full h-full overflow-hidden bg-white">
              <iframe 
                ...
                title="Nexus Google Sheets Bridge"
              />
           </div>
        </div>
        ...
        <div className="px-10 py-4 bg-black/80 border-t border-white/5 flex justify-between items-center text-[10px] font-mono text-white/10 uppercase italic shrink-0">
           <span>Protocolo: Nexus_DenseView_v1 // Cifrado: AES-256 // Zoom: 80%</span>
           <span className="flex gap-4">
              <span>LATENCY_SYNC: OK</span>
              <span>BUFFER_COMPENSATED</span>
           </span>
        </div>
      </div>
    </div>
