    ...
    return (
      <div className="space-y-1 relative">
        ...
        <div onClick={() => setIsOpen(!isOpen)} className="w-full glass bg-white/5 border border-white/10 rounded-2xl px-5 py-3 text-[11px] text-white cursor-pointer flex justify-between items-center hover:bg-white/10 transition-all">
           <span className="truncate text-white/90">{multi ? `${(value || []).length} seleccionadas` : (value || '---')}</span>
        ...
        <div className="max-w-5xl mx-auto space-y-10">
          ...
          {activeTab === 'core' && (
            <div className="space-y-10">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2"><label className="text-[9px] uppercase font-black text-white/20 ml-4">URL Home Assistant</label><input value={haConfig.url} onChange={e => setHaConfig({...haConfig, url: e.target.value})} className="w-full bg-white/5 p-4 rounded-2xl text-xs text-white outline-none border border-white/5" /></div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {haConfig.dashboardWidgets.map(w => (
                    <div key={w.id} className="glass p-5 rounded-3xl border border-white/10 space-y-3">
                       <div className="flex justify-between"><select value={w.type} onChange={e => setHaConfig({...haConfig, dashboardWidgets: haConfig.dashboardWidgets.map(x => x.id === w.id ? {...x, type: e.target.value as WidgetType} : x)})} className="bg-white/10 border-none text-[9px] uppercase font-bold text-blue-400 rounded-lg p-1 outline-none"><option value="sensor">Sensor</option><option value="switch">Interruptor</option><option value="button">Botón</option><option value="climate">Clima</option><option value="chart">Gráfico</option></select><button onClick={() => setHaConfig({...haConfig, dashboardWidgets: haConfig.dashboardWidgets.filter(x => x.id !== w.id)})} className="text-red-500/50">×</button></div>
                       <input value={w.title} onChange={e => setHaConfig({...haConfig, dashboardWidgets: haConfig.dashboardWidgets.map(x => x.id === w.id ? {...x, title: e.target.value} : x)})} placeholder="Título" className="w-full bg-white/5 p-2 rounded-xl text-[10px] text-white outline-none border border-white/5" />
          ...
          {activeTab === 'security' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               <EntitySelector label="Cámaras de Seguridad" value={haConfig.security_cameras} onChange={(v:any) => setHaConfig({...haConfig, security_cameras: v})} multi filterPrefix="camera." />
               <EntitySelector label="Sensores de Puertas/Ventanas" value={haConfig.security_sensors} onChange={(v:any) => setHaConfig({...haConfig, security_sensors: v})} multi filterPrefix="binary_sensor." />
               <EntitySelector label="Sensores de Temperatura" value={haConfig.temperature_sensors} onChange={(v:any) => setHaConfig({...haConfig, temperature_sensors: v})} multi filterPrefix="sensor." />
            </div>
          )}

          {activeTab === 'energy' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               <EntitySelector label="Producción Instantánea (W)" value={haConfig.solar_production_entity} onChange={(v:any) => setHaConfig({...haConfig, solar_production_entity: v})} />
               <EntitySelector label="Producción Diaria (kWh)" value={haConfig.solar_daily_entity} onChange={(v:any) => setHaConfig({...haConfig, solar_daily_entity: v})} />
               <EntitySelector label="Producción Mensual (kWh)" value={haConfig.solar_monthly_entity} onChange={(v:any) => setHaConfig({...haConfig, solar_monthly_entity: v})} />
               <EntitySelector label="Consumo Red Instantáneo (W)" value={haConfig.grid_consumption_entity} onChange={(v:any) => setHaConfig({...haConfig, grid_consumption_entity: v})} />
               <EntitySelector label="Exportación a Red (W)" value={haConfig.grid_export_entity} onChange={(v:any) => setHaConfig({...haConfig, grid_export_entity: v})} />
               ...
               <div className="col-span-full">
                 <EntitySelector label="Métricas de Energía Extra" value={haConfig.energy_extra_entities} onChange={(v:any) => setHaConfig({...haConfig, energy_extra_entities: v})} multi filterPrefix="sensor." />
               </div>
            </div>
          )}
          ...
          {activeTab === 'vehicle' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
               <EntitySelector label="Batería Vehículo (%)" value={haConfig.vehicle.battery_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, battery_entity: v}})} />
               <EntitySelector label="Autonomía Eléctrica (km)" value={haConfig.vehicle.range_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, range_entity: v}})} />
               ...
               <EntitySelector label="Nivel Gasolina (L)" value={haConfig.vehicle.fuel_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, fuel_entity: v}})} />
               <EntitySelector label="Autonomía Gasolina (km)" value={haConfig.vehicle.fuel_range_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, fuel_range_entity: v}})} />
               ...
               <EntitySelector label="Consumo Medio (kWh/100km)" value={haConfig.vehicle.avg_consumption_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, avg_consumption_entity: v}})} />
               <EntitySelector label="Estado Vehículo (Entity)" value={haConfig.vehicle.status_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, status_entity: v}})} />
               ...
               <EntitySelector label="Ventanas / Alarma" value={haConfig.vehicle.windows_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, windows_entity: v}})} />
               <EntitySelector label="Última Sincronización" value={haConfig.vehicle.last_update_entity} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, last_update_entity: v}})} />
               <div className="space-y-2">
                 <label className="text-[9px] uppercase font-black text-white/20 ml-4">URL Imagen Vehículo</label>
                 <input value={haConfig.vehicle.image_url} onChange={e => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, image_url: e.target.value}})} className="w-full bg-white/5 p-4 rounded-2xl text-xs text-white outline-none border border-white/5" />
               </div>
               <div className="space-y-2">
                 <label className="text-[9px] uppercase font-black text-white/20 ml-4">Script de Refresco (domain.service)</label>
                 <input value={haConfig.vehicle.refresh_script} onChange={e => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, refresh_script: e.target.value}})} className="w-full bg-white/5 p-4 rounded-2xl text-xs text-white outline-none border border-white/5" placeholder="script.refresh_lynk_co" />
               </div>
               <div className="col-span-full">
                 <EntitySelector label="Sensores Vehículo Adicionales" value={haConfig.vehicle.extra_entities} onChange={(v:any) => setHaConfig({...haConfig, vehicle: {...haConfig.vehicle, extra_entities: v}})} multi filterPrefix="sensor." />
               </div>
            </div>
          )}
          ...
          {activeTab === 'radar' && (
            <div className="space-y-6">
               <EntitySelector label="Personas a Rastrear" value={haConfig.tracked_people} onChange={(v:any) => setHaConfig({...haConfig, tracked_people: v})} multi filterPrefix="person." />
            </div>
          )}
          ...
          {activeTab === 'finance' && (
            <div className="space-y-6">
               <div className="flex items-center gap-4 mb-4">
                  <input type="checkbox" checked={haConfig.finance.use_sheets_mirror} onChange={e => setHaConfig({...haConfig, finance: {...haConfig.finance, use_sheets_mirror: e.target.checked}})} className="w-5 h-5 bg-blue-600 rounded" />
                  <label className="text-xs font-bold text-white">Usar Mirror de Google Sheets</label>
               </div>
               <div className="space-y-2">
                 <label className="text-[9px] uppercase font-black text-white/20 ml-4">CSV URL (Publicada de Sheets)</label>
                 <input value={haConfig.finance.sheets_csv_url} onChange={e => setHaConfig({...haConfig, finance: {...haConfig.finance, sheets_csv_url: e.target.value}})} className="w-full bg-white/5 p-4 rounded-2xl text-xs text-white outline-none" placeholder="https://docs.google.com/spreadsheets/d/.../pub?output=csv" />
               </div>
            </div>
          )}
          ...
          {activeTab === 'appearance' && (
            <div className="space-y-2">
               <label className="text-[9px] uppercase font-black text-white/20 ml-4">URL de Fondo Personalizado</label>
               <input value={haConfig.custom_bg_url} onChange={e => setHaConfig({...haConfig, custom_bg_url: e.target.value})} className="w-full bg-white/5 p-4 rounded-2xl text-xs text-white outline-none" placeholder="https://i.redd.it/6qq8lk9qjqp21.jpg" />
            </div>
          )}
