  ...
  return (
    <>
      ...
      <nav className="hidden md:flex flex-col w-24 bg-black/60 backdrop-blur-3xl border-r border-white/10 z-50 py-8 items-center h-screen shrink-0">
        ...
        <button onClick={onForceSync} className="p-4 text-white/10 hover:text-blue-400 transition-colors mt-2" title="Forzar Sincronización">
          <svg className="w-5 h-5 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
        </button>
        <button onClick={onLogout} className="p-4 text-white/10 hover:text-red-400 transition-colors mt-2 border-t border-white/5 w-full">
          <svg className="w-5 h-5 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7" /></svg>
        </button>
      </nav>
      
      {/* Mobile Tab Bar ultra-transparente para integración total */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-[calc(65px+env(safe-area-inset-bottom))] bg-black/20 backdrop-blur-xl border-t border-white/5 z-[100] flex items-start justify-around px-2 pt-2 pb-[env(safe-area-inset-bottom)]">
        {menuItems.map((item) => (
          <button key={item.id} onClick={() => onSectionChange(item.id)} className={`flex flex-col items-center justify-center min-w-[50px] h-12 gap-0.5 transition-all ${activeSection === item.id ? 'text-blue-400 scale-110' : 'text-white/20'}`}>
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={item.icon} /></svg>
