...
const VehicleView: React.FC = () => {
  ...
  const handleUpdateCar = async () => {
    ...
    let service = domain === 'button' ? 'press' : 'turn_on';
    await callHAService(config.url, config.token, domain, service, { entity_id: entityId });
  ...
  if (loading) return (
    <div className="h-[60vh] flex flex-col items-center justify-center">
       ...
       <p className="text-cyan-400 font-black text-[10px] uppercase tracking-[0.5em] animate-pulse">Sincronizando Sistemas de Abordo...</p>
    </div>
  );
  ...
  // Imagen Dinámica: Prioridad URL configurada -> Fallback
    ...
    <div className="flex flex-col gap-6 pb-24 animate-in fade-in zoom-in-95 duration-1000">
      ...
      <div className="relative glass rounded-[50px] overflow-hidden border border-white/10 h-[380px] md:h-[450px] shadow-2xl">
         <img 
            ...
            alt="Vehicle Image"
            onError={(e) => { 
               (e.target as any).src = 'https://images.unsplash.com/photo-1542362567-b05503f3af15?q=80&w=2000'; 
            }}
         />
         ...
         <div className="absolute top-10 left-10 md:top-14 md:left-14 space-y-4">
            <div className="flex items-center gap-3">
               ...
               <span className="text-[10px] font-black uppercase tracking-[0.6em] text-white/40">
                  {isCharging ? 'CARGA EN CURSO' : 'SISTEMA OPERATIVO'}
               </span>
            </div>
            <h2 className="text-5xl md:text-7xl font-black text-white italic tracking-tighter uppercase">NEXUS <span className="text-cyan-400">VEHICLE</span></h2>
            <p className="text-white/30 font-mono text-[10px] uppercase tracking-[0.4em]">Status: {getVal(config?.vehicle?.status_entity)}</p>
            
            <button 
              onClick={handleUpdateCar}
              disabled={isRefreshing}
              className="mt-6 px-8 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl flex items-center gap-4 transition-all group"
            >
              <svg className={`w-4 h-4 text-cyan-400 ${isRefreshing ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
              <span className="text-[9px] font-black uppercase tracking-widest text-white/60">Sincronizar Telemetría</span>
            </button>
         </div>
         ...
         <div className="absolute bottom-10 right-10 md:bottom-14 md:right-14">
            <div className="relative w-40 h-40 md:w-48 md:h-48">
               ...
               <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <p className="text-[8px] font-black uppercase tracking-widest text-white/20">Energía</p>
               </div>
            </div>
         </div>
      </div>

      {/* Grid de KPIs - Ahora dinámico con todos los valores */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
         {[
            ...
            { label: 'Carga Restante', val: getVal(config?.vehicle?.time_to_charge_entity), unit: 'min', color: 'text-yellow-400' },
         ].map((m, i) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <div className="glass p-8 rounded-[45px] border border-white/10 flex flex-col justify-between">
            <div className="flex justify-between items-start">
               <div>
                  <h3 className="text-[10px] font-black text-white/40 uppercase tracking-[0.4em] mb-1">Velocidad Carga</h3>
                  <p className="text-3xl font-black text-white">{getVal(config?.vehicle?.charging_speed_entity)} <span className="text-sm opacity-20">kW</span></p>
               </div>
               <div className="w-10 h-10 bg-cyan-400/10 rounded-xl flex items-center justify-center text-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.2)]">
            ...
            <div className="mt-8 space-y-4">
               <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                  <div className={`h-full bg-cyan-400 ${isCharging ? 'animate-pulse' : ''}`} style={{ width: isCharging ? '100%' : '0%' }} />
               </div>
            </div>
         </div>
         ...
         <div className="glass p-8 rounded-[45px] border border-white/10">
            ...
            <div className="space-y-6">
               <div className="flex justify-between items-end">
                  <div>
                     <p className="text-[9px] font-black uppercase tracking-widest text-white/20 mb-1">Total</p>
                     <p className="text-2xl font-black text-white">{getVal(config?.vehicle?.odometer_entity)} <span className="text-xs opacity-20">km</span></p>
                  </div>
                  <div className="text-right">
                     <p className="text-[9px] font-black uppercase tracking-widest text-white/20 mb-1">Servicio</p>
                     <p className="text-2xl font-black text-white/60">{getVal(config?.vehicle?.service_km_entity)} <span className="text-xs opacity-20">km</span></p>
                  </div>
               </div>
            </div>
         </div>
         ...
         <div className="glass p-8 rounded-[45px] border border-white/10 flex flex-col justify-between">
            <div>
               ...
               <div className="flex items-end gap-3">
                  ...
                  <span className="text-xl font-black text-white/20 mb-1 uppercase tracking-tighter">Litros</span>
               </div>
            </div>
            <div className="mt-6 flex justify-between items-center">
               <p className="text-[9px] font-black uppercase text-white/20 tracking-widest">Ahorro: <span className="text-white/60">{getVal(config?.vehicle?.saving_entity)}€</span></p>
            </div>
         </div>
      </div>
    </div>
