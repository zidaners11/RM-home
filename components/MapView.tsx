...
const MapView: React.FC = () => {
  ...
  const [people, setPeople] = useState<(UserLocation & { color: string, picture?: string, history: any[] })[]>([]);
    ...
    try {
      ...
      const peopleList = await Promise.all(
        peopleToTrack.map(async (id, index) => {
          ...
          // Historial de 24 horas como solicitado
          const historyData = await fetchHAHistory(cfg.url, cfg.token, id, 24);

          let lastLat: number | undefined = parseFloat(s.attributes?.latitude);
          let lastLng: number | undefined = parseFloat(s.attributes?.longitude);
          ...
          const history = (historyData || [])
            .map((h: any) => {
              ...
              if (!isNaN(lat)) lastLat = lat;
              if (!isNaN(lng)) lastLng = lng;

              return (lastLat !== undefined && lastLng !== undefined)
                ? { lat: lastLat, lng: lastLng, time: h.last_changed, state: h.state }
                : null;
            })
            .filter(Boolean)
            .sort((a: any, b: any) => new Date(a.time).getTime() - new Date(b.time).getTime());

          const picture = s.attributes?.entity_picture
            ? `${cfg.url.replace(/\/$/, '')}${s.attributes.entity_picture}`
            : `https://i.pravatar.cc/150?u=${id}`;

          const cLat = parseFloat(s.attributes?.latitude) || (history.length ? history[history.length - 1].lat : 0);
          const cLng = parseFloat(s.attributes?.longitude) || (history.length ? history[history.length - 1].lng : 0);
  ...
  return (
    <div className="flex flex-col gap-6 h-full pb-20 relative animate-in fade-in duration-1000">
      <style>{`
         ...
         .leaflet-tile-pane {
           filter: invert(100%) hue-rotate(180deg) brightness(0.4) contrast(1.2) grayscale(1) !important;
         }
       `}</style>

      <div className="glass rounded-[50px] overflow-hidden border border-white/10 flex-1 relative shadow-2xl bg-[#000000]">
        <div ref={mapContainerRef} className="w-full h-full z-0" />

        <div className="absolute top-8 left-8 flex flex-col gap-4 z-20">
          <div className="glass-dark px-6 py-6 rounded-[35px] border border-white/10 shadow-2xl min-w-[280px] backdrop-blur-3xl bg-black/80">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-[10px] font-black text-white uppercase tracking-[0.4em]">Sentinel Radar</h3>
                <p className="text-[7px] text-blue-400 font-bold uppercase mt-1 tracking-widest">24H History Active</p>
              </div>
            ...
            <div className="space-y-3">
              {people.length > 0 ? people.map(p => (
                ...
                <div className="py-10 text-center opacity-20">
                  <p className="text-[9px] font-black uppercase tracking-widest">Recuperando Telemetría...</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="absolute bottom-8 right-8 z-20">
          <div className="glass-dark px-6 py-4 rounded-2xl border border-white/5 text-[9px] font-black uppercase tracking-widest text-white/40 flex items-center gap-6 shadow-2xl bg-black/90">
